        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="basic-info-box col-md-4">
                    <div class="inner">
                        <h2>
                            @if ($project->isProcurementOrHigher())
    ...
    <div class="container content-project">
        ...
        <div id="download-page">
            ...
            @if (count($projectDocuments)>0)
                ...
                <!--<a href="{{route('project.all-files',['id'=>$project->id])}}" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect print hiddenAtPrint" target="_blank"><i class="material-icons">file_download</i>{{trans('general.download_page_documents')}}</a>-->
            @endif
        </div>
                        ...
                        @if ($project->projectInformation)
                            <ul>
                                ...
                                @if ($project->isTypePPP())
                                    ...
                                    <li>
                                        <h3>{{trans('project/project-information.project_summary_document')}}</h3>
                                        @if($project->projectInformation->project_summary_document && $project->projectInformation->project_summary_document_private)
                                            <p>{{$project->projectInformation->project_summary_document}}</p>
                                            @foreach($projectDocuments as $projectDocument)
                ...
                <div class="row">
                    ...
                    @if (isNigeriaSovereign())
                        ...
                        <div class="party-item col-sm-4">
                            @if($project->isTypePPP())
                                <h3 class="item-title">{{__('project.section.parties')}}</h3>
                            ...
                            @if($project->projectInformation->sponsor)
                                <img id="sponsor-logo" class="img-responsive img-centered party-logo" src="{{ route('uploader.par', array('position' => $project->projectInformation->sponsor->id)) }}" alt="{{$project->projectInformation->sponsor->name}}">
                            ...
                            <div class="col-sm-8">
                                <div class="row party-item">
                                    ...
                                    @foreach($project->parties as $party)
                                        ...
                                        <div class="col-sm-3">
                                            <img id="party-logo{{$party->id}}" class="party-logo img-centered" src="{{ route('uploader.par', array('position' => $party->id)) }}" alt="Logo">
                                            <div class="modal party-modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                                                <div class="modal-dialog modal-sm" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
            ...
            <div id="project-details" class="project-details"><!-- project-details -->
                ...
                <div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect"><!-- table1 -->
                    <div class="mdl-tabs__tab-bar hiddenAtPrint">
                        ...
                        @if ($project->isPDContractSummaryActive() && ($project->isTypePublic() || $project->isTypePrivate()))
                            <a href="#project-contract-summary-panel" class="mdl-tabs__tab is-active col-lg-3 col-md-3">{{__("project.section.project_details.contract-summary")}}</a>
                        @endif

                        @if ($project->isPDFinancialActive())
                            <a href="#financial-information-panel" class="mdl-tabs__tab col-lg-3 col-md-3 @if (!$project->isPDContractSummaryActive()) is-active @endif">{{__("project.financial-structure")}}</a>
                        @endif

                        @if ($project->isPDRenegotiationsActive())
                            <a href="#renegotiations-panel" class="col-md-2 mdl-tabs__tab @if (!$project->isPDContractSummaryActive() && !$project->isPDFinancialActive()) is-active @endif">{{__("project.section.project_details.renegotiations")}}</a>
                        @endif
                    </div>

                    @if ($project->isPDContractSummaryActive() && ($project->isTypePublic() || $project->isTypePrivate()))
                        <div class="mdl-tabs__panel is-active" id="project-contract-summary-panel">
                            <h3 class="showAtPrint printSectionHeader">{{__("project.section.project_details.contract-summary")}}</h3>
                            <ul>
                                @if(count($project->projectDetails->contractSummaries->where('published', true)) > 0)
                                    @foreach(collect($project->projectDetails->contractSummaries)->sortBy('position') as $contract_summary)
                                        @if($contract_summary->published)
                                            <li>
                                                <div class="panel-group wrap" id="accordion" role="tablist" aria-multiselectable="true">
                                                    <div class="panel">
                                                        <div class="panel-heading" role="tab" id="headingOne">
                                                            <h4 class="panel-title">
                                                                <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseDocument{{$contract_summary->id}}" aria-expanded="true" aria-controls="collapseDocument{{$contract_summary->id}}">{{$contract_summary->name}}</a>
                                                            </h4>
                                                        </div>
                                                        <div id="collapseDocument{{$contract_summary->id}}" class="panel-collapse collapse @if($loop->first) in @endif" role="tabpanel" aria-labelledby="headingOne">
                                                            <div class="panel-body">
                                                                <p>{{$contract_summary->description}}</p>
                                                                @foreach($projectDocuments as $projectDocument)
                                                                    @if ($projectDocument->section=="cs" && $projectDocument->position==$contract_summary->id)
                                                                        <a href="{{route('application.media',["id"=>$projectDocument->id])}}" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect project-button hiddenAtPrint"><i class="material-icons">file_download</i>{{$projectDocument->old_name}}</a>
                                                                    @endif
                                                                @endforeach
                                                            </div>
                    ...
                    @if ($project->isPDFinancialActive())
                        <div class="mdl-tabs__panel @if (!$project->isPDContractSummaryActive()) is-active @endif" id="financial-information-panel">
                    ...
                    @if ($project->isPDRenegotiationsActive())
                        <div class="mdl-tabs__panel @if (!$project->isPDContractSummaryActive() && !$project->isPDFinancialActive()) is-active @endif" id="renegotiations-panel">
                ...
                <div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect"><!-- table1 -->
                    <div class="mdl-tabs__tab-bar hiddenAtPrint">
                        @if ($project->isPDRisksActive() && ($project->isTypePublic() || $project->isTypePPP()))
                            <a href="#risks-panel" class="col-md-2 mdl-tabs__tab is-active">{{__("project.section.project_details.risks")}}</a>
                        @endif
                        @if ($project->isPDGovernmentSupportActive() && ($project->isTypePublic() || $project->isTypePPP()))
                            <a href="#government-support-panel" class="col-md-3  mdl-tabs__tab @if (!$project->isPDRisksActive()) is-active @endif">@if(isNigeriaSovereign() && !$project->isTypePublic()) {{__("project.section.project_details.government-support_ppp")}} @else {{__("project.section.project_details.government-support")}} @endif</a>
                        @endif
                        @if ($project->isPDTariffsActive() && $project->isTypePPP())
                            <a href="#tariffs-panel" class="col-md-2  mdl-tabs__tab @if (!$project->isPDRisksActive() && !$project->isPDGovernmentSupportActive()) is-active @endif">{{__("project.section.project_details.tariffs")}}</a>
                        @endif
                        @if ($project->isPDContractTerminationActive() && $project->isTypePPP())
                            <a href="#contract-termination-panel" class="col-md-3  mdl-tabs__tab @if (!$project->isPDRisksActive() && !$project->isPDGovernmentSupportActive() && !$project->isPDTariffsActive()) is-active @endif">{{__("project.terminal-provisions")}}</a>
                        @endif
                    </div>

                    @if ($project->isPDRisksActive() && ($project->isTypePublic() || $project->isTypePPP()))
                        <div class="mdl-tabs__panel is-active" id="risks-panel">
                    ...
                    @if ($project->isPDGovernmentSupportActive() && ($project->isTypePublic() || $project->isTypePPP()))
                        <div class="mdl-tabs__panel" id="government-support-panel">
                                    ...
                                    <div class="panel-group wrap" id="accordion" role="tablist" aria-multiselectable="true">
                                        <div class="panel">
                                            ...
                                            <div id="collapsectOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingctOne">
                                                <div class="panel-body">
                                                    @if($project->projectDetails->contractTerminations && count($project->projectDetails->contractTerminations->where('party_type', 'concessionaire')->where('published', true)) > 0)
                                                        <div class="table-responsive">
                                                            <table class="table-type2">
                                                                <thead>
                                                                <tr>
                                                                ...
                                                                <tbody>
                                                                @foreach($project->projectDetails->contractTerminations->where('party_type', 'concessionaire') as $contract)
                                    ...
                                    <div class="panel-group wrap" id="accordion" role="tablist" aria-multiselectable="true">
                                        <div class="panel">
                                            ...
                                            <div id="collapsectTwo" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingctTwo">
                                                <div class="panel-body">
                                                    @if($project->projectDetails->contractTerminations && count($project->projectDetails->contractTerminations->where('party_type', 'authority')->where('published', true)) > 0)
                                                        <div class="table-responsive">
                                                            <table class="table-type2">
                                                                <thead>
                                                                <tr>
                    ...
                    @if ($project->isPerformanceInformationActive())
                        ...
                        <div class="mdl-tabs__panel @if (!$project->performanceInformation->key_performance_active) is-active @endif" id="failures-panel">
                            ...
                            @if(count($project->performanceInformation->performanceFailures->where('published', true)) > 0)
                                ...
                                @foreach(array_chunk(collect($project->performanceInformation->performanceFailures)->sortBy('position')->where('published', true)->toArray(), 2) as $performanceFailures)
