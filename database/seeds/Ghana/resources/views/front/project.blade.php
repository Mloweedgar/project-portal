    ...
    <div class="container content-project"> <!-- content-project -->
        @if ($project->isPostprocurement() && $project->isProjectDetailsActive())
            <div id="project-details" class="project-details"><!-- project-details -->
                ...
                @if ($project->isPDFinancialActive() || $project->isPDDocumentsActive() || $project->isPDEnvironmentActive())
                    <div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect"><!-- table1 -->
                        <div class="mdl-tabs__tab-bar hiddenAtPrint">
                            @if ($project->isPDFinancialActive())
                                <a href="#financial-information-panel" class="mdl-tabs__tab is-active col-lg-3 col-md-3">{{__("project.financial-structure")}}</a>
                            @endif
                            @if ($project->isPDDocumentsActive())
                                <a href="#project-documents-panel" class="mdl-tabs__tab col-lg-3 col-md-3 @if(!$project->isPDFinancialActive()) is-active @endif">{{__("project.section.project_details.documents")}}</a>
                            @endif
                            @if ($project->isPDEnvironmentActive())
                                <a href="#environment-social-impact-assessment-panel" class="mdl-tabs__tab @if(!$project->isPDFinancialActive() && !$project->isPDDocumentsActive()) is-active @endif col-lg-6 col-md-6">{{__("project.section.project_details.environment_social_impact_assessment")}}</a>
                            @endif
                        </div>
                        ...
                        @if ($project->isPDDocumentsActive())
                            <div class="mdl-tabs__panel @if(!$project->isPDFinancialActive()) is-active @endif" id="project-documents-panel">
                        ...
                        @if ($project->isPDEnvironmentActive())
                            <div class="mdl-tabs__panel @if(!$project->isPDFinancialActive() && !$project->isPDDocumentsActive()) is-active @endif" id="environment-social-impact-assessment-panel">
                                <h3 class="showAtPrint printSectionHeader">{{__("project.section.project_details.environment_social_impact_assessment")}}</h3>
                                @if($project->projectDetails->environment->published)
                                    <p>{{ $project->projectDetails->environment->description }}</p>
                                    @foreach($projectDocuments as $projectDocument)
                                        @if ($projectDocument->section=="env")
                                            <a href="{{route('application.media',["id"=>$projectDocument->id])}}" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect project-button hiddenAtPrint"><i class="material-icons">file_download</i>{{$projectDocument->old_name}}</a>
                                        @endif
                                    @endforeach
                                @else
                ...
                @if ($project->isPDRisksActive() || $project->isPDGovernmentSupportActive() || $project->isPDTariffsActive() || $project->isPDContractTerminationActive() || $project->isPDRenegotiationsActive())
                    <div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect"><!-- table1 -->
                        <div class="mdl-tabs__tab-bar hiddenAtPrint">
                            ...
                            @if ($project->isPDGovernmentSupportActive())
                                <a href="#government-support-panel" class="col-md-3  mdl-tabs__tab @if (!$project->isPDRisksActive()) is-active @endif">{{__("project.section.project_details.government-support")}}</a>
                            @endif
                            ...
                            @if ($project->isPDRenegotiationsActive())
                                <a href="#renegotiations-panel" class="col-md-2 mdl-tabs__tab @if (!$project->isPDRisksActive() && !$project->isPDGovernmentSupportActive() && !$project->isPDTariffsActive() && !$project->isPDContractTerminationActive()) is-active @endif">{{__("project.section.project_details.renegotiations")}}</a>
                            @endif
                        </div>
                        ...
                        @if ($project->isPDGovernmentSupportActive())
                            <div class="mdl-tabs__panel @if (!$project->isPDRisksActive()) is-active @endif" id="government-support-panel">
                        ...
                        @if ($project->isPDTariffsActive())
                            <div class="mdl-tabs__panel @if (!$project->isPDRisksActive() && !$project->isPDGovernmentSupportActive()) is-active @endif" id="tariffs-panel">
                        ...
                        @if ($project->isPDContractTerminationActive())
                            <div class="mdl-tabs__panel @if (!$project->isPDRisksActive() && !$project->isPDGovernmentSupportActive() && !$project->isPDTariffsActive()) is-active @endif" id="contract-termination-panel"><!-- Terminal Provisions -->
                                        ...
                                        <div class="panel-group wrap" id="accordion" role="tablist" aria-multiselectable="true">
                                            <div class="panel">
                                                ...
                                                <div id="collapsectOne" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="headingctOne">
                                                    <div class="panel-body">
                                                        @if($project->projectDetails->contractTerminations && count($project->projectDetails->contractTerminations->where('party_type', 'concessionaire')) > 0)
                                                            <div class="table-responsive">
                                                                <table class="table-type2">
                                                                    <thead>
                                                                    <tr>
                ...
                <div class="mdl-tabs mdl-js-tabs mdl-js-ripple-effect"><!-- table2 -->
                    <div class="mdl-tabs__tab-bar hiddenAtPrint">
                        ...
                        @if ($project->isPIPerformanceFailuresActive())
                            <a href="#failures-panel" class="mdl-tabs__tab @if (!$project->isPIKeyPerformanceActive()) is-active @endif col-lg-4 col-md-4">{{trans("project.section.performance_information.performance_failures")}}</a>
                        @endif
                        @if ($project->isPIPerformanceAssessmentActive())
                            <a href="#assessments-panel" class="mdl-tabs__tab @if (!$project->isPIKeyPerformanceActive() && !$project->isPIPerformanceFailuresActive()) is-active @endif col-lg-4 col-md-4">{{trans("project.section.performance_information.performance-assessments")}}</a>
                        @endif
                    </div>
                    @if ($project->isPIKeyPerformanceActive())
                    ...
                    @if ($project->isPIPerformanceFailuresActive())
                        ...
                        <div class="mdl-tabs__panel @if (!$project->isPIKeyPerformanceActive()) is-active @endif" id="failures-panel">
                            ...
                            @if(count($project->performanceInformation->performanceFailures->where('published', true)) > 0)
                                ...
                                @foreach(array_chunk(collect($project->performanceInformation->performanceFailures)
                                ->sortBy('position')->toArray(), 2) as $performanceFailures)
                    ...
                    @endif
                    @if ($project->isPIPerformanceAssessmentActive())
                        <div class="mdl-tabs__panel @if (!$project->isPIKeyPerformanceActive() && !$project->isPIPerformanceFailuresActive()) is-active @endif" id="assessments-panel">
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(".party-item").find('img').click(function () {
            $(this).closest('.party-item').find('.modal').modal('show');
        });
