...
@section('content')
                ...
                <div class="body" data-status="0">
                    <form method="post" class="frmEditDocument" id="formCoreInfo" action="{{route("project.project-information.editCore")}}">
                        ...
                        <div class="row clearfix">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group form-float">
                                    <b>{{trans('general.contracting_authority')}}
                                    </b>
                                    <div class="form-line">
                        ...
                        <div class="col-sm-4">
                                <div class="form-group form-float">
                                    <b>{{trans('general.proposed_form_of_contract')}}</b>
                                    <div class="form-line">
                                        <select class="form-control show-tick" name="ppp_delivery_model" title="-- {{trans('general.choose-option')}} --">
                                            
                                            @foreach($ppp_delivery_models as $deliveryModel)
                                                <option value="{{$deliveryModel->id}}"
                                                        {{ (old("stage") == $deliveryModel->id ? "selected":"") }}
                                                        @if(json_decode($project_information)->ppp_delivery_model_id == $deliveryModel->id)
                                                        selected
                                                        @endif
                                                > {{$deliveryModel->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        @component(
                            'back.components.project-buttons', [
                                'section_fields' => ['sectors', 'regions', 'project_value_usd', 'stage', 'sponsor', 'project_value_second', 'ocid','cofog','indicative_timing_invest_unit','indicative_timing_invest_unit','ppp_delivery_model'],
                                'position'=>1,
                                'section'=>'i',
                                'project'=>$project->id,
    ...
    <!--
        PROJECT NEED
    -->
    @if ((Auth::user()->isAdmin() || Auth::user()->isProjectCoordinator()) || json_decode($project_information)->project_need_private)
    ...
    -->
    @if ( (Auth::user()->isAdmin() || Auth::user()->isProjectCoordinator())|| json_decode($project_information)->description_asset)
        <div class="row content-row toggleable-wrapper">
            <div class="col-md-12">
                <div class="card dynamic-card toggleable-container">
                    <div class="header card-header-static toggleable-card">
    ...
    -->
    @if ( (Auth::user()->isAdmin() || Auth::user()->isProjectCoordinator())|| json_decode($project_information)->description_services_private)
        <div class="row content-row toggleable-wrapper">
            <div class="col-md-12">
                <div class="card dynamic-card toggleable-container">
                    <div class="header card-header-static toggleable-card">
    ...
    -->
    @if ( (Auth::user()->isAdmin() || Auth::user()->isProjectCoordinator())|| json_decode($project_information)->reasons_ppp_private)
        <div class="row content-row toggleable-wrapper">
            <div class="col-md-12">
                <div class="card dynamic-card toggleable-container">
                    <div class="header card-header-static toggleable-card">
    ...
    -->
    {{--@if ( (Auth::user()->isAdmin() || Auth::user()->isProjectCoordinator())|| json_decode($project_information)->stakeholder_consultation_private)
        <div class="row content-row toggleable-wrapper">
            <div class="col-md-12">
                <div class="card dynamic-card toggleable-container">
                    <div class="header card-header-static toggleable-card">
    ...
    @endif --}}
    <!--
        PREFEASIBILITY REPORT DOCUMENT
    -->
    @if ( (Auth::user()->isAdmin() || Auth::user()->isProjectCoordinator())|| json_decode($project_information)->stakeholder_consultation_private)
        ...
        <div class="row content-row toggleable-wrapper">
            <div class="col-md-12">
                <div class="card dynamic-card toggleable-container">
                    <div class="header card-header-static toggleable-card">
                        <h2>
                            {{trans('project/project-information.prefeasibility_study_report')}}
                            @component('back.components.individual-visibility', [
                                'project' => $project->id,
                                'section' => 'i',
                    ...
                    <div class="body not-visible" data-status="0">
                        <form method="post" class="frmEditDocument" action="{{route("project.project-information.edit")}}">
                            ...
                            <div class="form-group">
                                <b>{{__("project/project-details/documents.description")}}</b>
                                <div class="form-line">
                                    <textarea rows="5" class="form-control no-resize richEditor" name="description" placeholder="{{trans('project/project-information.prefeasibility_study_report')}}">{{json_decode($project_information)->project_summary_document}}</textarea>
                                </div>
                            </div>
