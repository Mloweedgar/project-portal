...
@section('content')
    ...
    @if (!\Illuminate\Support\Facades\Auth::user()->isViewOnly())
        <div class="section-information">
            <a href="{{ route('documentation').'#contract_summary' }}"><i class="material-icons">help_outline</i> {{__('messages.section-info')}}</a>
        </div>
    @endif
    ...
    <h1 class="content-title-project-subsection">{{__("project/project-details/contract-summary.title")}}</h1>
    </div>
    @component('back.components.enable-subsection', ["visible" => $projectDetail->contract_summary_active])
    @endcomponent
        ...
        <div class="col-md-12">
            <div class="card card-shadow">
                <div id="card-header" class="header toggleable-card" @if (count($errors) > 0 || (isset($flag))) data-status="1" @else data-status="0" @endif>
                    <h2><i class="material-icons">add_box</i> <span>{{__("project/project-details/contract-summary.add-group")}}</span> <i id="keyboard_arrow" class="material-icons">keyboard_arrow_down</i></h2>
                </div>
                <div id="card-body" class="body @if (count($errors) > 0 || (isset($flag))) is-visible @else not-visible @endif">
                    ...
                    <form method="post" id="dynamic-form-validation" action="{{route("project-details-contract-summary/store")}}">
                        {{ csrf_field() }}
                        <input type="hidden" name="project_details_id" value="{{$projectDetail->id}}">
                        <div class="form-group">
                            <b>{{__("project/project-details/contract-summary.group")}}</b>
                            <div class="form-line">
                                <input type="text" class="form-control" name="name" placeholder="{{__("project/project-details/contract-summary.group-placeholder")}}" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <b>{{__("project/project-details/contract-summary.description")}}</b>
                            <div class="form-line">
                                <textarea class="form-control no-resize" name="description" placeholder="{{__("project/project-details/contract-summary.description-placeholder")}}" required></textarea>
                            </div>
                        </div>
    ...
    @if(count($contract_summaries) > 0)
        <ul id="sortable">

        @foreach($contract_summaries as $contract_summary)
                <li class="ui-state-default" data-order="{{$contract_summary->position}}">
                ...
                <div class="col-md-12">
                    <div class="card dynamic-card toggleable-container">
                        <div class="header card-header-static toggleable-card">
                            <h2>
                                @component('back.components.draft-chip',['draft'=>$contract_summary->draft])
                                @endcomponent
                                <span>{{$contract_summary->name}}</span>
                                @component('back.components.individual-visibility', [
                                    'project' => $project->id,
                                    'position' => $contract_summary->id,
                                    'status' => $contract_summary->published,
                                    'route' => route('project-details-contract-summary/visibility')
                                ])@endcomponent
                                <i class="fa fa-trash-o del-btn x2 delete-group" aria-hidden="true" data-id="{{$contract_summary->id}}"></i>
                            </h2>
                        </div>
                        <div class="body not-visible" data-status="0">
                            <form method="post" class="frmEditDocument" action="{{route("project-details-contract-summary/edit")}}">
                                {{ csrf_field() }}
                                <input type="hidden" name="id" value="{{$contract_summary->id}}">
                                <div class="form-group">
                                    <b>{{__("project/project-details/contract-summary.group")}}</b>
                                    <div class="form-line">
                                        <input type="text" class="form-control" name="name" placeholder="{{__("project/project-details/contract-summary.group-placeholder")}}" value="{{$contract_summary->name}}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <b>{{__("project/project-details/contract-summary.description")}}</b>
                                    <div class="form-line">
                                        <textarea class="form-control no-resize" name="description" placeholder="{{__("project/project-details/contract-summary.description-placeholder")}}">{{$contract_summary->description}}</textarea>
                                    </div>
                                </div>
                                ...
                                @component('components.uploader', [
                                    'projectAddress' => $project->id,
                                    'sectionAddress' => 'cs',
                                    'positionAddress' => $contract_summary->id
                                ])@endcomponent
...
@section('scripts')
    ...
    @component('back.components.enable-subsection-js', ["section" => "cs","project_id" => $projectDetail->id])
    @endcomponent
    ...
    <script>
        ...
        $("#order-save-button").click(function () {
            $.ajax({
                url: '{{ route('project-details-contract-summary/order') }}',
                type: 'POST',
    ...
    @endif
        ...
        @if(Auth::user()->canDelete())
            $('.delete-group').click(function(){
                ...
                swal({
                    ...
                    text: '{{ trans('project/project-details/contract-summary.delete_confirm') }}',
                    type: "warning",
                ...
                function(){
                    $.ajax({
                        url: '{{ route('project-details-contract-summary/delete') }}',
                        type: 'DELETE',
                        data: { id: id },
                        dataType: "json",
                        beforeSend: function() {
                        ...
                        success: function(data){
                            if (data.status) {
                                swal({
                                    title: '{{trans('messages.success')}}',
                                    text: '{{ trans('project/project-details/contract-summary.deleted') }}',
                                    type: "success",
                                    html: true
                                });
        ...
        @if (session('status') == true)
              swal({
                  title: "{{trans('messages.success')}}",
                  text: "{{trans('project/project-details/contract-summary.success')}}",
                  type: "success",
                  html: true
              }, function(){
              });
