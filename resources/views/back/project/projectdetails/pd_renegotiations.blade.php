...
@section('content')
    ...
    @if (!\Illuminate\Support\Facades\Auth::user()->isViewOnly())
        ...
        <a href="{{ route('documentation').'#renegotiations' }}"><i class="material-icons">help_outline</i> {{__('messages.section-info')}}</a>
    </div>
        @endif
    ...
    <div class="inline-block">
        <h1 class="content-title-project-subsection">{{__("project/project-details/renegotiations.title")}}</h1>
    </div>
    @component('back.components.enable-subsection', ["visible" => $projectDetail->renegotiations_active])
    @endcomponent
            ...
            <div class="col-md-12">
                <div class="card card-shadow">
                    <div id="card-header" class="header toggleable-card" @if (count($errors) > 0 || (isset($flag))) data-status="1" @else data-status="0" @endif>
                        <h2><i class="material-icons">add_box</i> <span>{{__("project/project-details/renegotiations.add-group")}}</span> <i id="keyboard_arrow" class="material-icons">keyboard_arrow_down</i></h2>
                    </div>
                    <div id="card-body" class="body @if (count($errors) > 0 || (isset($flag))) is-visible @else not-visible @endif">
                        ...
                        <form method="post" id="dynamic-form-validation" action="{{route("project-details-renegotiations/store")}}">
                            {{ csrf_field() }}
                            <input type="hidden" name="project_details_id" value="{{$projectDetail->id}}">
                            <div class="form-group">
                                <b>{{__("project/project-details/renegotiations.group")}}</b>
                                <div class="form-line">
                                    <input type="text" class="form-control" name="name" placeholder="{{__("project/project-details/renegotiations.placeholder")}}" required>
                                </div>
                            </div>
                            <div class="form-group">
                                <b>{{__("project/project-details/renegotiations.description")}}</b>
                                <div class="form-line">
                                    <textarea class="form-control no-resize" name="description" placeholder="{{__("project/project-details/renegotiations.description-placeholder")}}" required></textarea>
                                </div>
                            </div>
    ...
    @if(count($renegotiations) > 0)
        <ul id="sortable">

        @foreach($renegotiations as $renegotiation)
                <li class="ui-state-default" data-order="{{$renegotiation->position}}">
                ...
                <div class="col-md-12">
                    <div class="card dynamic-card toggleable-container">
                        <div class="header card-header-static toggleable-card">
                            <h2>
                                @component('back.components.draft-chip',['draft'=>$renegotiation->draft])
                                @endcomponent
                                <span>{{$renegotiation->name}}</span>
                                @component('back.components.individual-visibility', [
                                    'project' => $project->id,
                                    'position' => $renegotiation->id,
                                    'status' => $renegotiation->published,
                                    'route' => route('project-details-renegotiations/visibility')
                                ])@endcomponent
                                <i class="fa fa-trash-o del-btn x2 delete-group" aria-hidden="true" data-id="{{$renegotiation->id}}"></i></h2>
                        </div>
                        <div class="body not-visible" data-status="0">
                            <form method="post" class="frmEditDocument" action="{{route("project-details-renegotiations/edit")}}">
                                {{ csrf_field() }}
                                <input type="hidden" name="id" value="{{$renegotiation->id}}">
                                <div class="form-group">
                                    <b>{{__("project/project-details/renegotiations.group")}}</b>
                                    <div class="form-line">
                                        <input type="text" class="form-control" name="name" placeholder="{{__("project/project-details/renegotiations.placeholder")}}" value="{{$renegotiation->name}}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <b>{{__("project/project-details/renegotiations.description")}}</b>
                                    <div class="form-line">
                                        <textarea class="form-control no-resize" name="description" placeholder="{{__("project/project-details/renegotiations.description-placeholder")}}">{{$renegotiation->description}}</textarea>
                                    </div>
                                </div>
                                ...
                                @component('components.uploader', [
                                    'projectAddress' => $project->id,
                                    'sectionAddress' => 'r',
                                    'positionAddress' => $renegotiation->id
                                ])@endcomponent
...
@section('scripts')
    ...
    @component('back.components.enable-subsection-js', ["section" => "r","project_id" => $projectDetail->id])
    @endcomponent
    ...
    <script>
        ...
        $("#order-save-button").click(function () {
            $.ajax({
                url: '{{ route('project-details-renegotiations/order') }}',
                type: 'POST',
...
$('textarea, input, select').not('.morphsearch-input').prop('disabled', true);
        ...
        @if(Auth::user()->canDelete())
            ...
            swal({
                    ...
                    text: '{{ trans('project/project-details/renegotiations.delete_confirm') }}',
                    type: "warning",
                ...
                function(){
                    $.ajax({
                        url: '{{ route('project-details-renegotiations/delete') }}',
                        type: 'DELETE',
                        data: { id: id },
                        dataType: "json",
                        beforeSend: function() {
                        ...
                        success: function(data){
                            if (data.status) {
                                swal({
                                    title: '{{trans('messages.success')}}',
                                    text: '{{ trans('project/project-details/renegotiations.deleted') }}',
                                    type: "success",
                                    html: true
                                });
        ...
        @if (session('status') == true)
              swal({
            title: "{{trans('messages.success')}}",
            text: "{{trans('project/project-details/renegotiations.success')}}",
            type: "success",
            html: true
        }, function(){
        });
