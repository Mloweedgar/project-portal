    ...
    <div id="searching-area" class="row">
        <div class="col-md-2">
            <div data-open-first="true" class="accordion">
                ...
                <!-- NEW MENU -->
                <div class="accordion-item filter_menu_button" data-status="1">
                    <div class="accordion-title"><h4>Navigation<span id="left-navigation" class="label label-primary hidden">3</span></h4></div>
                    <div class="accordion-content">
                        <ul>
                                <li>
                                    <a href="/">
                                        <label id="lbl-home" class="">
                                            {{ trans('general.home') }}
                                        </label>
                                    </a>
                                </li>
                                <li>
                                    <a href="{{route('frontend.announcements')}}">
                                        <label id="lbl-home" class="">
                                            {{trans('menu.announcements')}} <span class="sr-only"></span>
                                        </label>
                                    </a>
                                </li>
                                <li>
                                    <div class="search-bar">
                                    @if (isset($projectInternalUrl) && $projectInternalUrl !== null)
                ...
                <div class="accordion-item active filter_menu_button" data-status="0">
        ...
        function drawMap() {
          ...
          var data = google.visualization.arrayToDataTable([
              ['State', 'Projects', {role: 'tooltip', p:{html:true}}],
                  @foreach ($regions as $region)
                    @if($region->name == "Zanzibar west")
                      ['Zanzibar west', {{$region->num_projects}}, '<b>Zanzibar west / Mjini Magharibi</b><br/><span>Projects: {{$region->num_projects}}</span>'],
                    @elseif($region->name == "Zanzibar north")
                      ['Zanzibar north', {{$region->num_projects}}, '<b>Zanzibar north / Kaskazini Unguja</b><br/><span>Projects: {{$region->num_projects}}</span>'],
                    @elseif($region->name == "Zanzibar south")
                      ['Zanzibar south', {{$region->num_projects}}, '<b>Zanzibar south / Kusini Unguja</b><br/><span>Projects: {{$region->num_projects}}</span>'],
                    @elseif($region->name == "Pemba north")
                      ['Pemba north', {{$region->num_projects}}, '<b>Pemba north / Kaskazini Pemba</b><br/><span>Projects: {{$region->num_projects}}</span>'],
                    @elseif($region->name == "Pemba south")
                      ['Pemba south', {{$region->num_projects}}, '<b>Pemba south / Kusini Pemba</b><br/><span>Projects: {{$region->num_projects}}</span>'],
                    @elseif($region->name == "Coast")
                      ['Coast', {{$region->num_projects}}, '<b>Coast / Pwani</b><br/><span>Projects: {{$region->num_projects}}</span>'],
                    @else
